...
欢迎大家参与编辑，或者[支持作者](#支持作者)。编写规范:

1. 保持题号升序，在对应位置插入
2. 题目名称使用 H2 标题 (两个 #)， 格式为 `("## %d %s (%dpts)", 题号, 题名, 分数)`
3. 记得更新目录，目录格式参考[这里](https://github.com/ekalinin/github-markdown-toc.go#remote-files)或下文说明

另附编写建议:

1. 写明输入输出格式. 能让人在不看网站上漫长的英文题面的情况下, 直接开始 coding
2. 标注数据范围, 除非原题就没写
3. 如果题目难理解, 可以适当增加对样例的解释
4. **最好在自己 AC 了该题目的情况下编写, 以保证自己理解的题意正确; 或者题意有模糊/歧义的地方一定要标注出来**

目录格式: `("[%s](#%s)", 标题, 链接)`，其中标题是展示出来的内容，应当和题目名称的 H2 标题保持一致。链接是通过标题转换的，大致规则是:
